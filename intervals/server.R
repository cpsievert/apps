...
shinyServer(function(input, output) {
  ...
  output$intervals <- renderPlot({
    if (input$repeated) {
      ...
      # Here we sample all possible means beforehand (and use some plotting trickery to only show a certain number)
      set.seed(1)
      ...
      p <- ggplot(df, aes(means, n)) + geom_point(aes(colour=colors)) + 
